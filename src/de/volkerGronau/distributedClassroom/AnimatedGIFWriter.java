...
// Taken from https://github.com/dragon66/animated-gif-writer
public class AnimatedGIFWriter {
	...
	public void writeAnimatedGIF(GIFFrame[] frames, OutputStream os) throws Exception {
		...
		for (int i = 0; i < frames.length; i++) {
			...
			writeFrame(pixels, imageWidth, imageHeight, frames[i].getLeftPosition(), frames[i].getTopPosition(), frames[i].getDelay(), frames[i].getDisposalMethod(), frames[i].getUserInputFlag(), os);
		}
	...
	public void writeFrame(OutputStream os, GIFFrame frame) throws Exception {
		...
		writeFrame(pixels, imageWidth, imageHeight, frame.getLeftPosition(), frame.getTopPosition(), frame.getDelay(), frame.getDisposalMethod(), frame.getUserInputFlag(), os);
	}
	...
	private static void dither_FloydSteinberg(int[] rgbTriplet, int width, int height, byte[] newPixels, int no_of_color, int[] colorPalette, int transparent_index) {
	...
	private static class WuQuant {
		...
		*/
		...
		private float Var(Box cube) {
			...
			dg = Vol(cube, mg);
			db = Vol(cube, mb);
			xx = m2[cube.r1][cube.g1][cube.b1] - m2[cube.r1][cube.g1][cube.b0] - m2[cube.r1][cube.g0][cube.b1] + m2[cube.r1][cube.g0][cube.b0] - m2[cube.r0][cube.g1][cube.b1] + m2[cube.r0][cube.g1][cube.b0] + m2[cube.r0][cube.g0][cube.b1] - m2[cube.r0][cube.g0][cube.b0];
	...
	private static class InverseColorMap {
		...
		public int getNearestColorIndex(int red, int green, int blue) {
			return invColorMap[(((red >> bitsDiscarded) << (bitsReserved << 1))) | ((green >> bitsDiscarded) << bitsReserved) | (blue >> bitsDiscarded)] & 0xff;
		}
